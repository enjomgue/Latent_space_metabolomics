---
title: "Lung cancer: Model Binary Classification - Xgboost classifier - 44 Factors"
author: "Evariste"
date: "2022-12-22"
...
print(imp_matrix)
print("Feature importance Plot : ")
xgb.plot.importance(importance_matrix = imp_matrix, rel_to_first = TRUE, xlab = "Relative importance", top_n=18, main="Factor Analysis: 44 factors - Top 18 features importance")
```
...
# Diagram factor
```{r}
loads = res_efa_44$rot_loadings

fa.diagram(loads[,2:3])
```

```{r}
which(substr(row.names(loads), 1, 9) == "MZ 264.12")
which(colnames(loads) == "F6")
```

# Analyze top 3 of each factors
```{r}
Top_1 = which(colnames(loads) == "F21")
paste("F21: ", sum(abs(loads[,Top_1]) > 0.766))
(loads[,Top_1])[abs(loads[,Top_1]) > 0.766]
#
Top_2 = which(colnames(loads) == "F3")
paste("F3: ", sum(abs(loads[,Top_2]) > 0.65))
(loads[,Top_2])[abs(loads[,Top_2]) > 0.65]
#
Top_3 = which(colnames(loads) == "F6")
paste("F6: ", sum(abs(loads[,Top_3]) > 0.89))
(loads[,Top_3])[abs(loads[,Top_3]) > 0.89]
#
Top_4 = which(colnames(loads) == "F14")
paste("F14: ", sum(abs(loads[,Top_4]) > 0.5))
(loads[,Top_4])[abs(loads[,Top_4]) > 0.5]
#
Top_5 = which(colnames(loads) == "F1")
paste("F1: ", sum(abs(loads[,Top_5]) > 0.918))
(loads[,Top_5])[abs(loads[,Top_5]) > 0.918]
```
...
for (i in 1:B.iter){
  ...
  # Precision recall
  pr_auc = pr.curve(test_data[, 1], pred_proba_test_tune, curve=FALSE)
