---
title: "Lung Cancer: Run High dimensional discriminant analysis (HDDA) with all features"
...
#(test_data[,1])[1:10]
#posterior[1:10,]
#(posterior[,1])[test_data[,1] == 1]
#length(posterior[which(test_data[,1] == 1), 2])
#posterior[1]
#test_data[,1]

pr_auc = roc.curve(test_data[, 1], posterior[,1], curve=TRUE)
pr_auc

#pr_auc = pr.curve((posterior[,2])[test_data[,1] == 1], (posterior[,2])[test_data[,1] == 0],                     curve=TRUE)
plot(pr_auc)
```
...
for (i in 1:B.iter){
  ...
  # Accuracy
  B.accuracy[i] = mean(res$class == test_data[,1])
  # AUC
  res.roc_tune <- roc(test_data[, 1], posterior[,1])
